package com.alex.asyncexamples.server;
...
import com.alex.util.NamedThreadFactory;
/**
 * ServerManager is responsible for starting, stopping and restarting AysncCounterServers.
 * <P>
 * Clients should never directly interact with CounterServer instances, instead they send messages
 * over TCP / IP. Tests or Server programs to start Servers should use this API to manage Servers.
 * <P>
 * ServerManager is a singleton and follow Singleton pattern as described by Josh Bloch
 * Item 3, Effective Java Second Edition. This approach provides guarentees against multiple 
 * instantiation even in the face of sophisticated serialization or reflection attacks.
 * <P>
 * ServerManager contains some simple Server Management logic such as checking that a specified 
 * Server is not already started when a request is made to start it. 
 * <P>
 * ServerManager has its own thread pool. Servers are started on these threads. 
 * <P>
 * ServerManager can write counter values to a file. This is useful for example if you have to restart a server.
 * If a Server is the first Server to start up it will read from the file. 
 * <P>
 * <dev-note>
 * Only allowing starting and stopping via the ServerManager simplifies threading concerns.  ServerManager takes a
 * coarse grained locking approach when starting and stopping servers. For example, it will lock when starting a server
 * befores it read any countervalue from a file, then it will check the server is up and running before returning the lock.
 * It will do similar when stopping a server.  Preventing some complex races conditions.
 * <P>
 */
public enum ServerManager {
	...
	private String counterFileName = "counterValue.txt";  // counter file name with default value.  might be changed for tests etc.
